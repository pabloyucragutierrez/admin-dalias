...
import type { Product, ProductDto, Category, Branch, Brand, Unit, FilterOptionsProducts } from '@/interfaces/products.interface';
...
export const activeOrInactiveProduct = async (id: string, payload: StatusDto) => {
  try {
    const response = await api.patch(`/products/inactivoOrActivo/${id}`, payload);
    return response.data as ApiResponse<Product>;
  } catch (e) {
    console.error(e);
    throw e;
