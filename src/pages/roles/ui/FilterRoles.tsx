...
import type { FilterOptionsRoles } from "@/interfaces/roles.interface";

interface FilterRolesProps {
  onApplyFilters: (filters: FilterOptionsRoles) => void;
  onClearFilters: () => void;
  initialFilters?: FilterOptionsRoles;
  loading: boolean;
}
...
const FilterRoles: React.FC<FilterRolesProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const nonEmptyFilters: FilterOptionsRoles = {};
    Object.entries(filters).forEach(([key, value]) => {
      if (value && value.trim() !== "") {
        nonEmptyFilters[key as keyof FilterOptionsRoles] = value.trim();
      }
    });
        ...
        {Object.entries(initialFilters).map(([key, value]) =>
          value ? (
            ...
            >
              ...
              {key === "state" && value === "true" ? "Activo" : value === "false" ? "Inactivo" : value}
            </div>
          ) : null
        )}
