...
interface LineasTableProps {
  ...
  loading: boolean;
  hasMore: boolean;
  fetchMoreLineas: () => void;
  selectedLineas: string[];
  toggleLineaSelection: (id: string) => void;
  selectAllLineas: () => void;
  changeStatusFn: (linea: Linea) => void;
}
...
const LineasTable: React.FC<LineasTableProps> = ({
  lineas,
  loading,
  hasMore,
  fetchMoreLineas,
  ...
  const lastLineaRef = useCallback(
    ...
    [loading, hasMore, fetchMoreLineas]
  );

  const uniqueLineas = useMemo(() => {
    const seen = new Set();
    return lineas.filter((linea) => {
      const duplicate = seen.has(linea.id);
      seen.add(linea.id);
      return !duplicate;
    });
  }, [lineas]);
        ...
        <div className="max-h-[600px] overflow-y-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {uniqueLineas.map((linea, index) => (
                <tr
                  key={`${linea.id}-${index}`}
                  ref={
                    index === uniqueLineas.length - 1 ? lastLineaRef : null
                  }
                  className={
                    selectedLineas.includes(linea.id) ? "bg-blue-50" : ""
                  }
                >
                  {columnVisibility.name && (
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button
                        ...
                        onClick={() => navigate(`/lineas/${linea.id}`)}
                      >
                        {linea.name}
                      </button>
                    </td>
                  ...
                  {columnVisibility.createAt && (
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-500">
                        {formatDateTime(linea.createAt)}
                      </div>
                    </td>
                  )}
                  <td className="px-6 py-4 whitespace-nowrap">
                    <Badge variant={linea.status ? "success" : "destructive"}>
                      {linea.status ? "Activa" : "Inactiva"}
                    </Badge>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
